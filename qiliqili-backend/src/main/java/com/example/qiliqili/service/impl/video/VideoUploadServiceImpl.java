...
import com.example.qiliqili.mapper.VideoStatsMapper;
import com.example.qiliqili.pojo.CustomResponse;
import com.example.qiliqili.pojo.Video;
import com.example.qiliqili.pojo.VideoStats;
import com.example.qiliqili.pojo.dto.VideoUploadInfoDTO;
import com.example.qiliqili.service.utils.CurrentUser;
import com.example.qiliqili.service.video.VideoUploadService;
import com.example.qiliqili.utils.ESUtil;
...
//        customResponse.setData(counts);
     ...
     */
    @Override
    public CustomResponse uploadChunk(MultipartFile chunk, String hash, Integer index) throws Exception {

        log.info("Uploading chunk " + hash + " to " + CHUNK_DIRECTORY);
        CustomResponse customResponse = new CustomResponse();
        // 构建分片文件名
...
//        chunk.transferTo(chunkFile);
        minioUtils.uploadChunk(chunk,hash,index);
...
//        ossUploadUtil.deleteFiles("chunk/", hash + "-");
     ...
     */
    @Override
    public CustomResponse addVideo(MultipartFile cover, VideoUploadInfoDTO videoUploadInfoDTO) throws Exception {
        ...
        //保存封面到minio
        String coverUrl  = minioUtils.uploadVideoCover(cover,videoUploadInfoDTO.getHash());
        // 保存封面到本地
//        try {
//            // 获取当前时间戳
...
//        String coverUrl = fileUploadUtils.upload(UploadEnum.ARTICLE_IMAGE,cover);
...
//                mergeChunks(videoUploadInfoDTO);
                String videoURL = minioUtils.completeMultipartUpload(videoUploadInfoDTO.getHash());
                // 存入数据库
                Date now = new Date();
