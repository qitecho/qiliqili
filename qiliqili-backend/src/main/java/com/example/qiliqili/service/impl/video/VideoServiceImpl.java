...
import com.example.qiliqili.service.video.VideoStatsService;
...
//        System.out.println("封装耗时：" + (end - start));
    ...
    public Map<String, Object> getVideoWithDataById(Integer vid) {
        Map<String, Object> map = new HashMap<>();
        // 先查询 redis
        log.info("getVideoWithDataById: " + vid);
        ...
        userFuture.join();
        categoryFuture.join();
        log.info("getVideoWithDataById: map" + map);
        return map;
    }
...
//                  CompletableFuture.runAsync(() -> fileUploadUtils.deleteFile("videoPrefix","ds"), taskExecutor);
//                    CompletableFuture.runAsync(() -> fileUploadUtils.deleteFile("coverPrefix","s"), taskExecutor);
