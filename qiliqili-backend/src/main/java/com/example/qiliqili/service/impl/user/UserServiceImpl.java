...
import com.example.qiliqili.pojo.dto.UserDTO;
...
//                    userDTO.setPlayCount(play);
    ...
    @Override
    public CustomResponse updateUserAvatar(Integer uid, MultipartFile file) throws Exception {
        // 保存封面到OSS，返回URL
//        String avatar_url = ossUtil.uploadImage(file, "avatar");

        String avatar_url = minioUtils.uploadUserAvatar(uid,file);
        // 查旧的头像地址
...
//                ossUtil.deleteFiles(filename);
                minioUtils.deleteFile(userAvatarBucket,filename);
            }
        }, taskExecutor);
